...
class GCClient:
	...
	def __init__(self, img, k,mask):
		...
		FG_ind=np.where(mask==255)
		PrFG_ind=np.where(mask==122)
		self._mask[FG_ind]=self._GC_FGD
		self._mask[PrFG_ind]=self._GC_PR_FGD
...
if __name__ == '__main__':
	img = cv2.imread('../data/fgMask/fg3.png', cv2.IMREAD_COLOR)
	mask=cv2.imread('../data/fgMask/newmask3.png',cv2.IMREAD_COLOR)
	output = np.zeros(img.shape,np.uint8)

	GC = GCClient(img, 5,mask)
	GC.run()
